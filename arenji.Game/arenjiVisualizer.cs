...
namespace arenji.Game
{
    public partial class arenjiVisualizer : Screen, IKeyBindingHandler<ArenjiAction>
    {
        ...
        {   
            //handling the new request taken from the menu, instead of the drag and drop
            if (projectPrompt == null)
            {
                Schedule(() => HandleDroppedFile(filePath));
                return;
            }
            ...
            {
                string targetMidi = filePath.EndsWith(".ini") 
                    ? Directory.GetFiles(directory, "*.mid").FirstOrDefault() 
                    : filePath;

                if (targetMidi != null)
                {
                    arenjiProjectManager.CurrentProjectFolder = directory; 
                    string loadedMidiPath = arenjiProjectManager.LoadProject(existingIniPath, settingsPanel);
                    settingsPanel.RefreshUIAfterLoad(); 

                    targetMidi = string.IsNullOrEmpty(loadedMidiPath) ? targetMidi : loadedMidiPath;
